...
% Perform Spectral decomposition and select Eigen-vectors with lasrgest 
XtX = data * data';
[V,D] = eig(XtX);
new_data = V(:,(end-2):end);
...
[cluster_labels, data_centroids, iter] = Kmean_Learner(new_data, 3, 10, 0.000001);


%%% Plot new clusters
myColors = zeros(size(new_data,1),3);
